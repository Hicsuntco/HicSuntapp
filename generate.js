...
function buildSkeletonPrompt(dc, batchSize, offset){
  ...
  if(isFirst){
    ...
    return [
      ...
      '- EXEMPLE SARDAIGNE (nord→sud) : Olbia(arr.) → Costa Smeralda → Nuoro/Orgosolo → Barumini → Cagliari → Chia → Villasimius. OU (sud→nord) : Cagliari(arr.) → Chia → Oristano/Sinis → Bosa → Alghero → Sassari → Castelsardo. JAMAIS Cagliari → Olbia → Cagliari.',
      '- EXEMPLE SARDAIGNE 4j : Cagliari(arr.) → Chia/sud → Oristano/Sinis → Alghero. OU Olbia(arr.) → Costa Smeralda → Nuoro → Cagliari. Circuit logique nord-sud OU sud-nord, PAS de zigzag.',
      '- Sur une ÎLE (Sardaigne, Bali, Sri Lanka…) : tracer un circuit en boucle ou en ligne droite nord↔sud ou est↔ouest. Ne jamais traverser l\'île deux fois au même endroit.',
