...
function _promptSaveModified(){
  ...
  ov.innerHTML = '<div style="background:var(--surface-raised);border-radius:24px 24px 0 0;padding:28px 22px calc(28px + env(safe-area-inset-bottom,0px));width:100%;max-width:520px">'
    ...
    + '<button onclick="_doSaveModified(\'replace\')" style="width:100%;background:transparent;color:var(--ink);border:1.5px solid var(--ink);border-radius:14px;padding:15px;font-family:var(--sans);font-size:15px;font-weight:700;cursor:pointer;margin-bottom:10px">Remplacer l\'ancienne version</button>'
  ...
  if(items===null){
    ...
    if(!token && !email){
      host.innerHTML = '<div style="text-align:center;padding:60px 24px">'
        ...
        + '<button onclick="setTab(\'create\')" style="background:transparent;color:var(--ink);border:1.5px solid var(--ink);border-radius:14px;padding:14px 28px;font-family:var(--sans);font-size:15px;font-weight:700;cursor:pointer">Composer un itinéraire</button>'
        + '</div>';
    } else {
  ...
  window.bookingView = function(accId){
    ...
    function platformRow(url, logo, color, title, subtitle, found){
      return '<a href="'+url+'" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:14px;padding:16px;background:var(--surface);border:1px solid '+(found?accent:'var(--line)')+';border-radius:14px;margin-bottom:10px;text-decoration:none;cursor:pointer">'
        ...
        +'<div style="flex:1"><div style="font-size:15px;font-weight:500;color:var(--ink)">'+title+(found?' <span style="font-size:10px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;color:'+accent+';border:1px solid '+accent+';border-radius:6px;padding:2px 6px;margin-left:6px">Vérifié ici</span>':'')+'</div><div style="font-size:12px;color:var(--sub)">'+subtitle+'</div></div>'
    ...
    return '<div class="book-hero" style="position:relative;overflow:hidden;height:200px;background:linear-gradient(155deg,#1c1812,#0d0b08)">'
      ...
      +'<p style="font-size:13px;color:var(--sub);margin-bottom:16px;line-height:1.5">'
        +(src==='officiel'
          ? 'Cet hébergement a été vérifié sur son site officiel — comparez aussi ses tarifs sur ces plateformes.'
          : (src ? 'Cet hébergement a été vérifié disponible sur la plateforme mise en avant ci-dessous — comparez aussi sur les autres.' : 'Cliquez pour voir les disponibilités et les vrais prix sur vos dates — choisissez la plateforme la moins chère.'))
      +'</p>'
      +platformsHtml
      +'</div>';
  };
    ...
    try{
      ...
      if(changed){
        ...
        var actionsHTML = '<div style="margin:10px 0 4px 40px;display:flex;flex-direction:column;gap:8px;align-items:flex-start">'
          + '<button onclick="_returnToUpdatedItinerary()" style="background:transparent;color:var(--ink);border:1.5px solid var(--ink);border-radius:12px;padding:10px 16px;font-family:var(--sans);font-size:13px;font-weight:700;cursor:pointer">Voir l\'itinéraire mis à jour →</button>'
